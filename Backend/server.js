...
const generateReferralId = () => {
    return crypto.randomBytes(5).toString('hex').toUpperCase(); // Generates a 10-character alphanumeric ID
};
...
app.post('/api/applications', upload.fields([
    ...
    try {
        ...
        while (!isUnique) {
            ...
            const checkReferral = await pool.query(
                'SELECT id FROM applications WHERE referral_id = $1',
                [referralId]
            );
        ...
        const result = await pool.query(
            `INSERT INTO applications (
                referral_id, role, location, personal_info, employment_status, 
        ...
        res.status(201).json({ id: result.rows[0].id, referral_id: result.rows[0].referral_id, message: 'Application submitted successfully' });
    } catch (error) {
        ...
        if (search || status !== 'all') {
            ...
            if (search) {
                conditions.push(
                    ...
                    OR role ILIKE $1
                    OR referral_id ILIKE $1)` // Added referral_id to search
                );
                values.push(`%${search}%`);
            }
